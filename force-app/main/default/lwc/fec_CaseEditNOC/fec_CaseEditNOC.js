...
//PhongBT11 update jira KH-1084 bổ sung Updated Information cho NOC, GSR Handling Stage
  ...
  handleCaseNOCMessage(message) {
    ...
    if (message.fecRepublishBusinessNoc === true) {
      if (
        ...
        getNatureOfCase({
          ...
          .catch((error) => {
            console.log(
              "🚀 ~ Fec_CaseEditNOC ~ fecRepublishBusinessNoc ~ getNatureOfCase:",
              error
            );
          });
      }
  ...
  async handlePublishMessageChanel(isUserAction = false) {
    const payload = {
      caseId: this.recordId,
      productTypeId: this.productTypeSelectedId,
      categoryId: this.categorySelectedId,
  ...
  handleMessage(message) {
    ...
    // Chỉ tab đúng Case — tránh tin từ Case khác ghi đè mode / reloadData
    if (message.caseId != null && message.caseId !== this.recordId) {
      return;
    }
...
//PhongBT11 update jira KH-1084 bổ sung Updated Information cho NOC, GSR Handling Stage
  ...
  getSubCode() {
    getSubCodelst({
      ...
      .then((res) => {
        ...
        // Khi quay lại tab Case, fec_CaseBussiness có thể mount lại và getData() chỉ đọc DB;
        // combo NOC vẫn đúng theo state/Case nhưng luồng (dynamic LWC) cần CASE_NOC + getData đủ tham số.
        if (
          this.productTypeSelectedId &&
          this.categorySelectedId &&
          this.subCategorySelectedId &&
          ...
          getNatureOfCase({
            ...
            .catch((error) => {
              console.log(
                "🚀 ~ Fec_CaseEditNOC ~ getSubCode ~ sync getNatureOfCase:",
                error
              );
            });
        }
  ...
  handleChangeSubCategory(e) {
    ...
    if (this.subCategorySelectedId) this.handlePublishMessageChanel(true);
  }
  ...
  handleChangeSubCode(e) {
    ...
    if (this.subCodeSelectedId) {
      getNatureOfCase({
        ...
        .then((result) => {
          this.natureOfCase = result;
          this.handlePublishMessageChanel(true);
        })
        .catch((error) => {
          console.log("error", error);
        });
