...
// ── Channels Tab: Column Definitions ──────────────────────────────────
...
// ── Properties Tab: Column Definitions ───────────────────────────────
...
export default class FecLiveDataView extends LightningElement {
    selectedItem = {};
    ...
    handleRefreshTree() {
        ...
        if (treeComp) {
            treeComp.refreshTreeData();
        }
    }

    // ── Channels Tab: Wire Adapters ───────────────────────────────────
    ...
    wiredChannels({ data, error }) {
        ...
        if (data) {
            ...
            this.channels = [];
            this.showErrorToast(error.body?.message || error.message || 'Error loading channels');
        }
    }
    ...
    wiredCaseChannels({ data, error }) {
        ...
        if (data) {
            ...
            this.caseChannels = [];
            this.showErrorToast(error.body?.message || error.message || 'Error loading case channels');
        }
    }

    // ── Properties Tab: Wire Adapter ──────────────────────────────────
    ...
    wiredAdditionalFields({ data, error }) {
        ...
        if (data) {
            ...
            this.showErrorToast(error.body?.message || error.message || 'Error loading additional fields');
        }
    }

    // ── Properties Tab: Row Select → Load Sub-Table ──────────────────
    handleFieldRowSelect(event) {
        ...
        getLiveFieldListValues({ fieldId: row.Id })
            .then(result => {
                this.fieldListValues = result || [];
            })
            .catch(error => {
                this.fieldListValues = [];
                this.showErrorToast(error.body?.message || error.message || 'Error loading list values');
            })
            .finally(() => {
                this.isLoadingListValues = false;
            });
    }
    ...
    get hasCaseChannels() {
        return this.caseChannels && this.caseChannels.length > 0;
    }
    ...
    get hasFieldListValues() {
        return this.fieldListValues && this.fieldListValues.length > 0;
    }

    get showFieldListValuesSection() {
        return this.selectedFieldId != null;
    }

    get fieldListValuesTitle() {
        return this.selectedFieldName
            ? `List Values: ${this.selectedFieldName}`
            : 'List Values';
    }
