...
export default class Fec_InteractionHighlight extends LightningElement {
  ...
  get maskedNationalId() {
    ...
    if (this.isNationalIdMasked) {
      ...
      const first3 = nationalId.substring(0, 3);
      const last3 = nationalId.substring(nationalId.length - 3);
  ...
  get eyeIcon() {
    return this.isPhoneMasked ? 'utility:preview' : 'utility:hide';
  }

  get nationalIdEyeIcon() {
    return this.isNationalIdMasked ? 'utility:preview' : 'utility:hide';
  }
  ...
  handleTogglePhone() {
    ...
    if (wasMasked && !this.isPhoneMasked && this.recordId) {
      logSensitiveAccess({
        ...
        console.error('Error logging sensitive access for Primary Phone:', error);
      });
    }
  }
  ...
  handleToggleNationalId() {
    ...
    if (wasMasked && !this.isNationalIdMasked && this.recordId) {
      logSensitiveAccess({
        ...
        console.error('Error logging sensitive access for National ID/ Passport ID:', error);
      });
    }
  }
