...
export default class FecSelectAttachment extends LightningElement {
    ...
    wiredAttachments({ error, data }) {
        if (data) {
            ...
            this.showToast('Error', 'Could not load attachments: ' + error.body.message, 'error');
            console.error(error);
        }
    }
    ...
    async handleAttachFiles() {
        if (this.selectedContentDocumentIds.size === 0) {
            this.showToast('Warning', 'Vui lòng chọn ít nhất một tệp đính kèm mới.', 'warning');
            return;
        }
        ...
        try {
            ...
            if (result) {
                this.showToast('Success', 'Đã gắn tệp đính kèm thành công.', 'success');
                // Đóng modal và refresh giao diện
                this.closeModal();
            } else {
                this.showToast('Error', 'Có lỗi xảy ra khi gắn tệp.', 'error');
            }
        } catch (error) {
            this.showToast('Error', 'Lỗi hệ thống: ' + error.body.message, 'error');
        }
    }
