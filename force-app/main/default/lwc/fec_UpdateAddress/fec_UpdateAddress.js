...
import initializeCasePendingAddressesFromMasters from '@salesforce/apex/FEC_MainInfoController.initializeCasePendingAddressesFromMasters';
...
import saveCaseSelectedAddressInfo from '@salesforce/apex/FEC_MainInfoController.saveCaseSelectedAddressInfo';
...
export default class Fec_UpdateAddress extends LightningElement {
    ...
    /**
     * Gọi từ fec_CaseBussiness khi Process Action / Submit thành công:
     * reload Main Info (áp Case.FEC_Updated_Info_* lên cột Updated), giữ snapshot Original.
     * Không refresh C360 / không sync master — CIF có thể đã cập nhật nhưng Original snapshot giữ nguyên.
     */
    @api refreshUpdatedInformationAfterProcessSuccess() {
        ...
        const preserveSnapshot = this.originalSnapshotInitialized === true;
        ...
        return loadMainInfo({ caseId: this.resolvedCaseId })
            .then((data) => {
                ...
                if (preserveSnapshot && Array.isArray(preservedSnapshot)) {
    ...
    loadAddressData() {
        ...
        return initializeCasePendingAddressesFromMasters({
            caseId: this.resolvedCaseId
        })
            .catch(() => {
                /* vẫn load Main Info nếu init pending thất bại */
            })
            .then(() => loadMainInfo({ caseId: this.resolvedCaseId }))
            .then((data) => {
    ...
    /**
     * Case.FEC_Updated_Info_* (sau Save/Submit) — ưu tiên cao hơn pending clone từ master.
     */
    _applyCaseUpdatedInfoTextsToDisplay(data) {
        const updatedEntries = [
            { text: data?.updatedInfoPermanentAddress, sfType: TYPE_PERMANENT },
            { text: data?.updatedInfoOfficeAddress, sfType: TYPE_OFFICE },
            { text: data?.updatedInfoCurrentAddress, sfType: TYPE_CURRENT }
        ];
        const addresses = Array.isArray(this.mainInfoData?.addresses)
            ? this.mainInfoData.addresses.map((a) => ({ ...a }))
            : [];
        let dirty = false;
        for (const { text, sfType } of updatedEntries) {
            if (!text || String(text).trim() === '') {
                continue;
            }
            const composed = String(text).trim();
            const idx = addresses.findIndex((a) => a && a.addressType === sfType);
            if (idx >= 0) {
    ...
    /**
     * Checkbox mailing cột Updated: chỉ bật khi đang edit đúng dòng đó.
     */
    ...
    get newAddrMailingCheckboxDisabled() {
        return this.newAddressSaveLoading || this.newAddressModalLoading || this.isLoading;
    }
    ...
    async _persistLastEditedAddressSelection(sfAddressType, pendingRowId) {
        ...
        this.mainInfoData = {
            ...(this.mainInfoData || {}),
            selectedAddressInfoId: selectedId
        };
        /* Không syncMailingSelectionFromData: field này = loại vừa Save cho API, không đổi checkbox mailing. */
    }
    ...
    _labelsFromOriginalAddressForType(sfAddressType) {
        ...
        if (full) {
            ...
            if (!districtLabel && parts.length >= 2) {
                const wardFromAddress = this.stripTrailingCountryForDisplay(
                    parts[parts.length - 2]
                );
                if (
                    wardFromAddress &&
                    wardFromAddress.toLowerCase() !== cityLabel.toLowerCase() &&
                    !this._isCodeLikePicklistLabel(wardFromAddress, wardFromAddress)
                ) {
                    districtLabel = wardFromAddress;
                }
            }
        }
        return { districtLabel, cityLabel };
    ...
    _labelFromOptions(options, value) {
        ...
        for (const o of list) {
            ...
            if (ov === v) {
                ...
                if (lab && !this._isCodeLikePicklistLabel(lab, v)) {
                    return lab;
                }
                return '';
            }
    ...
    /**
     * Nhãn Ward cho combobox edit: ưu tiên districtLabel từ CIF/pending JSON,
     * sau đó orig.ward / chuỗi địa chỉ gốc.
     */
    _resolveDistrictLabelForEdit(sfAddressType, primarySource) {
        const fromPrimary =
            primarySource?.districtLabel != null
                ? String(primarySource.districtLabel).trim()
                : '';
        if (fromPrimary) {
            return fromPrimary;
        }
        return this._labelsFromOriginalAddressForType(sfAddressType).districtLabel || '';
    }

    /** Label chỉ là mã (trùng value hoặc toàn số) — không dùng làm nhãn hiển thị. */
    _isCodeLikePicklistLabel(label, value) {
    ...
    ensurePicklistValue(options, value, preferredLabel) {
        ...
        const displayLabel =
            preferred &&
            !this._isCodeLikePicklistLabel(preferred, v) &&
            preferred !== v
                ? preferred
                : '';
        ...
        if (idx >= 0) {
            const cur = list[idx];
            const curLabel = cur.label != null ? String(cur.label).trim() : '';
            if (displayLabel && this._isCodeLikePicklistLabel(curLabel, v)) {
                list[idx] = { ...cur, label: displayLabel, value: v };
            }
            return list;
        }

        const fallbackLabel = displayLabel || '';
        if (!fallbackLabel) {
            return list;
        }
        return [...list, { label: fallbackLabel, value: v }];
    }
                ...
                .then((norm) => {
                    ...
                    if (this.mailingCity && Array.isArray(districts) && districts.length > 0) {
                        ...
                        this.districtOptions = this.ensurePicklistValue(
                            dOpts,
                            this.mailingDistrict,
                            this._resolveDistrictLabelForEdit(
                                sfAddressType,
                                pendingData
                            )
                        );
                    ...
                    if (!hadDistrictInJson && hasCity) {
                        return getMailingAddressUpdateContext({
                            ...
                            this.districtOptions = this.ensurePicklistValue(
                                this.districtOptions,
                                this.mailingDistrict,
                                this._resolveDistrictLabelForEdit(sfAddressType, {
                                    districtLabel:
                                        ctx.districtLabel ||
                                        pendingData.districtLabel
                                })
                            );
                    ...
                    if (this.mailingCity) {
                        return getDistrictOptionsForProvinceCode({
                            ...
                            this.districtOptions = this.ensurePicklistValue(
                                dOpts,
                                this.mailingDistrict,
                                this._resolveDistrictLabelForEdit(sfAddressType, ctx)
                            );
                        });
                    }
                    if (
    ...
    async handleSaveMailing() {
        ...
        try {
            ...
            const editedRow = this.rowFromSfAddressType(primarySfAddressType);
            const isMailingForSave =
                selectedRow != null && selectedRow === editedRow ? 'Y' : 'N';
            const primaryInfo = {
                caseId: this.resolvedCaseId,
                ...
                isMailingAddress: isMailingForSave
            };
            const pendingRowId = await this._savePendingAddressRecord({
                sfAddressType: primarySfAddressType,
                deletePending: false,
                parentAddressInfoId:
    ...
    async handleSaveNewAddress() {
        ...
        try {
            ...
            const primaryInfo = {
                ...
                isMailingAddress: nextSelectedRow === primaryRow ? 'Y' : 'N'
            };
